...
- Prompt PERF1B adds `docs/performance/RUNTIME_PERFORMANCE_ARCHITECTURE_AUDIT.md`; fixes a P0 unbounded Worker-ingress family plus stale-revision, completion, startup, failure, lifecycle, selected-state, and abstraction defects; and proves bounded transport, transfer ownership, deterministic Local/Worker/direct equivalence, three-minute soaks, repeated lifecycle, and production isolation. It leaves only explicit `flocking-v1` projection support, does not migrate production World, and does not start A0 or I1.
