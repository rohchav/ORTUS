...
GW0 does not implement routes, navigation, World/Lab/Atlas/Workshop pages, persistence, accounts, cloud storage, local storage, database schemas, progression state, unlocks, XP, levels, achievements, badges, streaks, missions, quests, daily rewards, discovery detection, regime classification, behavioral landscapes, contextual recommendations, notebooks, saved research assets, model composition, grand-system scenarios, design tokens, CSS, components, runtime behavior, simulation behavior, template behavior, dependencies, assets, or mockups. GW1 implements only the first structural destination shell. GW2 implements only live World active-run provenance and observation context. GW2B audits and hardens that layer without adding product behavior. GW3 implements only live World intervention readiness and model-response boundary context. GW3B audits and hardens that layer without adding product behavior. GW4 implements only non-persistent Atlas information architecture and evidence semantics. GW4B audits and hardens that Atlas foundation without adding product behavior. GW5 implements only non-persistent Lab evidence-record information architecture and experiment-ledger semantics. GW5B audits and hardens that Lab foundation without adding product behavior. GW6 remains future and must not start without explicit direction.
