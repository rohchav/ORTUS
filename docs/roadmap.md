...
- Prompt GW6 adds static source-backed contextual capability guidance with `src/lib/capabilityGuidance.ts`, `src/components/researchWorld/CapabilityGuidancePanel.tsx`, and `docs/ui/CONTEXTUAL_CAPABILITY_GUIDANCE.md`. World, Workshop, Lab, and Atlas now show route-local guidance for available, planning-only, not implemented, do-not-assume, and related-destination boundaries. Capability guidance describes current product capability; it does not create capability. It does not implement saved records, Atlas discoveries, Lab experiments, behavioral landscapes, progression, generated advice, persistence, runtime behavior, template behavior, Builder execution, dependencies, validation, calibration, or real-world discovery certification.
