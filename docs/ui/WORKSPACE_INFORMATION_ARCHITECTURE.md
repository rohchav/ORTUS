...
Lab is a reachable non-persistent GW5 foundation, not a saved evidence-record system. Atlas is a reachable non-persistent GW4/GW7 foundation. Atlas now includes behavioral-landscape vocabulary and a conceptual scaffold, but Lab and Atlas do not contain persistent experiments, notebooks, saved Discovery Atlas records, saved behavioral landscape maps, progression, fake counts, fake maps, fake records, or fake user activity.
...
- Prompt GW7 behavioral-landscape foundation is static Atlas-side vocabulary from `src/lib/behavioralLandscapeFoundation.ts`. It does not subscribe to simulation ticks, mutate active simulation state, execute Builder structures, run sweeps, persist landscape state, create Lab records, create Atlas records, or detect regimes.
