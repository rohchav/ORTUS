...
Lab is a reachable non-persistent GW5 foundation, not a saved evidence-record system. Atlas is a reachable non-persistent GW4/GW7/GW8 foundation, audited through GW8B. Atlas now includes behavioral-landscape vocabulary, landscape probe planning vocabulary, and conceptual scaffolds, but Lab and Atlas do not contain persistent experiments, notebooks, saved Discovery Atlas records, saved behavioral landscape maps, saved probe plans, sampled data, run queues, run sweeps, regime detection, progression, fake counts, fake maps, fake records, or fake user activity.

Prompt UX3 audits the rendered hierarchy and concludes that the current routes are honest but still too console-like for the Living Systems Atlas sandbox/workbench mission. UX3 complete. GW9 paused until the sandbox-theme and guided-comprehension track is addressed or explicitly waived. Next recommended prompt: UX4. The recommended sequence is UX4, UX4B, UX5, UX5B, UX6, UX6B, then GW9. UX3 does not create Beginner Mode, Advanced Mode, Guided Builder, preferences, persistence, production layout changes, runtime behavior, or route behavior.
