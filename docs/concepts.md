...
Prompt GW0 is documentation and progression architecture only. The canonical Research World progression roadmap is `RESEARCH_WORLD_PROGRESSION_MINI_ROADMAP.md`. A Research World is a model-bounded investigation context, not a literal game world, complete real-world domain simulation, achievement layer, persistence implementation, or runtime capability. GW0 defines World/Lab/Atlas/Workshop destination responsibilities, progression without XP, Discovery Atlas and Behavioral Landscape boundaries, reusable-asset provenance, expert access, and the sequence now completed through GW9B. It does not implement routes, navigation, persistence, discovery logic, saved behavioral landscapes, contextual guidance, UI/CSS changes, dependencies, assets, runtime behavior, or template behavior.

Prompt UX4 is a bounded visual-language implementation prompt. The canonical source is `ui/SANDBOX_VISUAL_LANGUAGE_FOUNDATION.md`. UX4 retunes shared semantic surfaces, panel/frame styling, status/caveat treatment, route hierarchy, World model-surface presentation, Workshop bench styling, and Lab/Atlas static scaffold grouping. It changes visual language only. It does not add persistence, preferences, onboarding, progressive disclosure, Guided Builder, route aliases, Lab records, Atlas discoveries, samples, run queues, sweeps, regime detection, runtime behavior, template behavior, Builder execution, dependencies, assets, fonts, icon libraries, validation, calibration, or GW9 implementation.

Prompt UX4B is a bounded audit and hardening prompt. The canonical audit record is `ui/SANDBOX_VISUAL_LANGUAGE_FOUNDATION_AUDIT.md`. UX4B audits the UX4 visual-language foundation and finds it ready for UX5. It does not add production behavior, runtime behavior, template behavior, Builder execution, persistence, progression, onboarding, Lab records, Atlas discoveries, samples, route aliases, dependencies, assets, fonts, icon libraries, UX5, UX6, or GW9 implementation.

Prompt UX5 is a bounded progressive-disclosure and information-architecture implementation. The canonical source is `ui/PROGRESSIVE_DISCLOSURE_AND_BEGINNER_ADVANCED_INFORMATION_ARCHITECTURE.md`. UX5 adds source-backed plain-language route orientation, component-local non-persistent disclosures, layered capability guidance, and concise Lab/Atlas defaults while keeping exact technical vocabulary and expert tools directly reachable. Workshop remains the Advanced Builder. UX5 adds no Guided Builder, stored beginner/advanced mode, preference, personalization, recommendation, onboarding state, progression, runtime/template/Builder execution, Lab/Atlas persistence, landscape sampling, or probe execution.

Prompt UX5B is a bounded source and rendered audit. The canonical audit is `ui/PROGRESSIVE_DISCLOSURE_AND_BEGINNER_ADVANCED_INFORMATION_ARCHITECTURE_AUDIT.md`. UX5B finds the covered hierarchy, language, expert access, local disclosure state, reload defaults, landmarks, responsive behavior, and capability boundaries ready for UX6. It adds no production behavior, Guided Builder, persistence, personalization, progression, runtime/template/Builder execution, Lab/Atlas behavior, sampling, probe execution, or GW9 behavior.

Prompt RH1 is a documentation and repo-hygiene prompt. The canonical hygiene record is `RH1_REMOTE_STATE_AND_MISSION_ALIGNMENT_HYGIENE.md`. RH1 verifies local `main` is ahead of `origin/main`, documents that the GitHub remote was stale before the RH1 continuation push, and clarifies two source-of-truth distinctions: existing local World comparison storage may preserve bounded run summaries, but it is not persistent Lab evidence, Atlas discovery storage, saved behavioral landscapes, saved probe plans, or real-world validation; existing Experiment Runner sweeps support bounded local model comparison, but they are not Atlas landscape sampling, landscape probe execution, saved sampled regions, run queues, or regime detection.
...
Prompt GW9 adds one explicit bounded Atlas runtime preview for `flocking-boids`; Prompt GW9B audits and hardens that slice in `ui/EPHEMERAL_LANDSCAPE_SAMPLING_PREVIEW_V1_AUDIT.md`. A canonical preview request executes fresh template engines at exact declared coordinates and retains temporary model-output provenance. It is not generic landscape or probe execution, a saved research record, interpolation, regime detection, statistical confidence, scientific validation, or a real-world prediction. GW9B does not widen template/scenario support or add persistence.
...
Prompt UX6B audits and hardens this boundary in `ui/GUIDED_BUILDER_AND_STEP_BY_STEP_CONFIGURATION_FLOW_FOUNDATION_AUDIT.md`. It fixes generated note-id bounds, direct mode-switch focus, canceled-handoff status, responsive Workshop content flow, repeated Advanced landmark names, nested scenario-plan scrolling, and Graph sidebar keyboard scrolling. It does not broaden the Guided subset, make schemas runnable, change World, add storage, or implement GW9.
