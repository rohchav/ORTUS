...
Parameter controls are generated from template definitions. Numeric controls show current values and ranges, and changes are applied by rebuilding the run through engine validation so invalid parameter combinations do not enter the engine. When Neural Excitation is selected, Setup also shows the Neural Runtime Lab before exact numeric parameters; the lab provides scenario cards, mission/status readouts, bounded live explanations, direct template-scoped actions, plain-English setup mappings, a bounded timeline, and a non-adaptive RPS challenge shell. Exact Neural parameters remain available in Advanced config. The Micro panel also includes an agent avatar display preference for canvas-only rendering styles: glyphs, arrows, initials, or head markers. File exchange reports whether a scenario or snapshot export/import succeeded.
...
Neural Runtime Lab direct actions are discoverable UI shortcuts over supported template interventions or fresh-run rebuilds. They do not add selected-edge editing, hidden schema execution, Builder graph execution, Model Schema execution, learning, opponent modeling, payoff-driven weight updates, or strategy adaptation.
