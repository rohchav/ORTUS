...
The current Research World shell source of truth is `docs/ui/RESEARCH_WORLD_DESTINATION_SHELL.md`. GW1 persistence refers to persistent application structure across routes, not persistent user research data. The route contract is `/` for World, `/lab` for Lab, `/atlas` for Atlas, and `/builder` for Workshop. Lab and Atlas are future-only informational destinations; they are reachable, not locked, and they contain no fake experiments, discoveries, maps, counts, notebooks, saved assets, or progress state. The GW1 rendered continuation fixed the skip link so keyboard focus is immediately visible inside the viewport.
