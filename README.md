...
- An isolated, unlinked I0/I0B comparison route at `/world/immersive-prototype` with three real-engine Flocking presentation prototypes: Living Diorama, God-Hand, and Field Scientist. The audited route uses selected-only detail, automatic bounded presentation quality, and a read-only scene adapter. It is non-persistent prototype evidence, not the production World replacement or cross-template renderer support.
