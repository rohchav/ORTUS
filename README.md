...
- A task-centered Start Hub at `/` with one featured Flocking starter, four ways to begin, and a seven-system catalog derived from authoritative production-template metadata.
...
`npm run test:ui` is a dev-only Playwright/Axe rendered audit harness for `/`, `/world`, `/builder`, `/lab`, and `/atlas`. R1/R1B cover Start Hub, fresh featured-starter handoff, model dominance, explicit parameter-rebuild semantics, compact Setup, concise Understand, action-first Atlas, quiet Workshop/Lab hierarchy, task/query coherence, keyboard focus, required viewports, storage, diagnostics, reduced motion, and Axe while preserving the earlier Research World, Guided Builder, and Atlas execution contracts. These checks are not a beginner user study, screen-reader, assistive-technology, forced-colors, actual browser-zoom, mobile-workflow, or WCAG conformance claim.

## Roadmap Status

ORTUS has completed Prompt 39B plus the documented neural, Research World, and UI work through GW9B, followed by `R1: Start Hub and World-First Product Reset` and `R1B: First-Run and World-First Shell Audit + Hardening`. The active product sequence is documented in `docs/product/ORTUS_PRODUCT_EXPERIENCE_RESET_ROADMAP.md`. `R2: World Layout and Interaction Reclaim` is next. The F0 fractal branch remains documented but F1 is paused under the future E3 Analytical Lenses milestone.
...
The historical Research World shell source of truth is `docs/ui/RESEARCH_WORLD_DESTINATION_SHELL.md`, with the GW1B audit record in `docs/ui/RESEARCH_WORLD_DESTINATION_SHELL_AUDIT.md`. R1 supersedes its route hierarchy: `/` is Start, `/world` is World, `/builder` is Workshop, `/atlas` is Atlas, and `/lab` is Lab. This route reset changes product packaging, not persistent user research data or runtime behavior. The R1 implementation record is `docs/ui/START_HUB_AND_WORLD_FIRST_PRODUCT_RESET.md`; the R1B rendered audit and hardening record is `docs/ui/FIRST_RUN_AND_WORLD_FIRST_SHELL_AUDIT.md`.
