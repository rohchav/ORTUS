...
`npm run test:ui` is a dev-only Playwright/Axe rendered audit harness for `/`, `/world`, `/builder`, `/lab`, and `/atlas`. The current suite contains `110` tests. R1/R1B cover the Start Hub and first-run handoff; R2 covers the stable World stage, compact context, task/query coherence, bounded active-tool scrolling, Setup layering, Observe hierarchy, current-run Change semantics, Compare, model-specific Explain/full reference, tool collapse/restore, required viewports, keyboard focus, reduced motion, diagnostics, and Axe. These checks are not a beginner user study, screen-reader, assistive-technology, forced-colors, actual browser-zoom, complete mobile-workflow, or WCAG conformance claim.

## Roadmap Status

ORTUS has completed Prompt 39B plus the documented neural, Research World, and UI work through GW9B, followed by `R1: Start Hub and World-First Product Reset`, `R1B: First-Run and World-First Shell Audit + Hardening`, and `R2: World Layout and Interaction Reclaim`. The active product sequence is documented in `docs/product/ORTUS_PRODUCT_EXPERIENCE_RESET_ROADMAP.md`. `R2B: World Layout and Interaction Audit` is next and has not started. The F0 fractal branch remains documented but F1 is paused under the future E3 Analytical Lenses milestone.
