...
ORTUS is a browser-based complex-systems simulation sandbox backed by a deterministic TypeScript engine.
...
- One optional guided investigation, `Reading a Flock`, at `/worlds/guides/reading-a-flock`. It organizes the existing clear/noisy Flocking pair, World tasks, playback, outputs, and Compare workflow without auto-running, auto-saving, scoring, or storing progress. C3B hardens authority drift, modified-run disclosure, comparison provenance, and hostile URL handling without changing simulation behavior.
- A browser World route at `/world` for running and observing hand-built template-owned simulations, with the live model dominant and persistent playback controls. Production Flocking uses the audited dedicated Worker runtime and bounded `flocking-v1` presentation projections; the other six templates retain the explicit legacy main-thread path.
- An isolated, unlinked I0/I0B/PERF1/PERF1B comparison route at `/world/immersive-prototype` with three real-engine Flocking presentation prototypes: Living Diorama, God-Hand, and Field Scientist. It remains non-persistent historical prototype evidence, not another production authority or cross-template Worker/renderer support.
...
`npm run lint` runs TypeScript with unused-symbol checks plus a scoped AST-based architecture/accessibility smoke gate. It checks static imports and re-exports, CommonJS and dynamic module loading, current simulation/UI and research-authority boundaries, seeded-randomness references, dynamic-execution forms, selected platform globals, and a narrow intrinsic-JSX keyboard baseline. It is not full dependency or data-flow analysis, a complete ESLint ruleset, a browser audit, a screen-reader test, or WCAG certification.

`npm run test:ui` is a dev-only Playwright/Axe rendered audit harness for `/`, `/worlds`, `/worlds/[slug]`, `/worlds/packs/[slug]`, `/worlds/guides/[slug]`, `/world`, `/world/immersive-prototype`, `/builder`, `/lab`, and `/atlas`. R1/R1B cover the Start Hub and first-run handoff; R2/R2B cover the stable World stage and task architecture; C1/C1B cover the original seven-world framework; C2/C2B cover the first flagship collection and eight strict recipe handoffs; C3/C3B cover the optional `Reading a Flock` flow; I0/I0B cover the isolated real-Flocking concept comparison; PERF1/PERF1B cover the runtime architecture; and I1/I1B cover production Flocking adoption, replacement/reset/provenance, recoverable rejection, Worker absence, keyboard operation, constrained viewports, high DPR, reduced motion, and bounded 100/500 characterization. These checks are not a beginner user study, educational-outcome test, screen-reader, assistive-technology, forced-colors, actual browser-zoom, complete touch-workflow, browser/mobile hardware survey, or WCAG conformance claim.
...
Canonical architecture vocabulary is defined in `docs/ARCHITECTURE.md`; `docs/concepts.md` is a detailed subordinate implementation vocabulary. In short: templates define current hand-built model families, scenarios define initial conditions and supported variants, runs execute a template/scenario/seed configuration, `SimulationSnapshotView` supplies a detached broad read model, `SnapshotExport` restores exact continuation state, run summaries compare bounded outcomes, experiments batch runs, and interventions perturb a live run through validated engine paths.
...
Future sequence is canonical only in `docs/ROADMAP.md`. The lowercase `docs/roadmap.md` and `docs/missing-pillars.md` preserve historical prompt-era planning and do not override it.
...
This legacy structural navigation service is not the planned scientific `SystemViewSpec` graph. Likewise, the isolated immersive prototype's button and function names containing “System view” reset a presentation camera only; they do not select, derive, or execute a scientific SystemView.
...
- `SnapshotExport` stores current tick/time, world state, events, RNG streams, metrics history, applied intervention history, and metadata. Importing a validated snapshot export restores the current run; a `SimulationSnapshotView` cannot.
...
Production World has an explicit split runtime. Flocking uses one dedicated Worker-owned engine, seeded RNG, runtime scheduler, and command path. Canvas consumes bounded transferable `RenderFramePacket` data; React consumes coarse `UIProjection` and one selected-detail projection. The visual loop does not publish full snapshots through Zustand, and production code has no hidden Local fallback. Generation/run/template identity and strictly increasing revisions gate adoption; frame/UI gates retain at most one in-flight plus one newest pending value; and requests/controls share a maximum of 128 unconsumed Worker messages. Invalid bounded requests are recoverable rejections, while terminal Worker/protocol failure stops execution visibly.

The six non-Flocking templates retain the mounted React animation-frame accumulator, main-thread engine, detached `SimulationSnapshotView`, Zustand controls, and batched Canvas path. This is explicit legacy coupling, not generic Worker support. Across both paths the engine/runtime owns step order, modeled time, mutation, and seeded randomness; presentation does not. The Debug panel can show developer-only timing data when performance instrumentation is enabled with `localStorage.setItem("ortus.performanceInstrumentation.v1", "enabled")`; remove that key or set any other value to disable it. Instrumentation records bounded tick, metric, snapshot, frame/update, entity-count, neighbor-query, forest-fire, flocking, and named duration data without changing simulation semantics.
