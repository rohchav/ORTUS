...
- An isolated, unlinked I0/I0B/PERF1 comparison route at `/world/immersive-prototype` with three real-engine Flocking presentation prototypes: Living Diorama, God-Hand, and Field Scientist. PERF1 runs its deterministic engine in a dedicated Worker, transfers compact render frames, publishes coarse React state, and keeps detail selected-only. It remains non-persistent prototype evidence, not the production World replacement or cross-template Worker/renderer support.
