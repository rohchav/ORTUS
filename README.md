...
- An isolated, unlinked I0/I0B/PERF1/PERF1B comparison route at `/world/immersive-prototype` with three real-engine Flocking presentation prototypes: Living Diorama, God-Hand, and Field Scientist. Its audited `flocking-v1` runtime can run the same deterministic session through Local or dedicated Worker drivers, transfer compact render frames, publish coarse React state, and keep detail selected-only. It remains non-persistent prototype evidence, not the production World replacement or cross-template Worker/renderer support.
...
ORTUS has completed Prompt 39B plus the documented neural, Research World, and UI work through GW9B, followed by R1 through C3B, I0/I0B, PERF1/PERF1B, A0, and `A0B: Canonical Architecture + Source-of-Truth Audit`. `I1: Production Runtime Migration + Immersive Shell Foundation` is next and unstarted. I2 through I5B are not an unconditional sequence, and `C4: Flagship Starter Pack Two` is not tied to I5B. `docs/ROADMAP.md` is the canonical sequence. The F0 fractal branch remains historical planning, with F1 paused under future E3 analytical-lens prerequisites.
...
Production World currently uses a mounted React animation-frame accumulator to decide wall-clock cadence and request fixed engine steps through Zustand. The engine owns each step's system order, command application, modeled-time increment, mutation, and seeded randomness. When ticks advance, the store publishes one detached `SimulationSnapshotView` per animation frame and the World Stage renders it through a single Canvas pass. PERF1/PERF1B leave that coupled production workflow intact; only the isolated Flocking prototype uses the dedicated runtime scheduler/Worker, compact renderer packets, and coarse UI projections. Its audited runtime lifecycle is explicit, stale same-run revisions are rejected, frame/UI gates retain at most one in-flight plus one newest pending value, and all requests/controls share a maximum of 128 unconsumed Worker messages. The Debug panel can show developer-only timing data when performance instrumentation is enabled with `localStorage.setItem("ortus.performanceInstrumentation.v1", "enabled")`; remove that key or set any other value to disable it. Instrumentation records bounded tick, metric, snapshot, frame/update, entity-count, neighbor-query, forest-fire, flocking, and named duration data without changing simulation semantics.
