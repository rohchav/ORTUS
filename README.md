...
`npm run test:ui` is a dev-only Playwright/Axe rendered audit harness for `/`, `/world`, `/builder`, `/lab`, and `/atlas`. R1 adds Start Hub, featured-starter handoff, model-dominance, compact Setup, concise Understand, action-first Atlas, quiet Workshop, Lab hierarchy, menu keyboard, required-viewport, storage, focus, diagnostics, reduced-motion, and Axe coverage while preserving the earlier Research World, Guided Builder, and Atlas execution contracts. These checks are not a beginner user study, screen-reader, assistive-technology, forced-colors, actual browser-zoom, mobile-workflow, or WCAG conformance claim.

## Roadmap Status

ORTUS has completed Prompt 39B plus the documented neural, Research World, and UI work through GW9B, followed by `R1: Start Hub and World-First Product Reset`. The active product sequence is documented in `docs/product/ORTUS_PRODUCT_EXPERIENCE_RESET_ROADMAP.md`. R1B First-Run and Shell Audit is next. The F0 fractal branch remains documented but F1 is paused under the future E3 Analytical Lenses milestone.
