...
Neural Runtime Lab UX V1 is a React workbench layer around existing Neural template parameters, snapshots, and template-scoped interventions. It does not change the headless simulation engine, add a new primitive, execute model schemas, execute Builder graphs, add learning/adaptation, or update weights/biases from RPS payoff. This lab shows stylized neural excitation dynamics and bounded categorical readouts. It does not model cognition, biological neurons, or learning. RPS payoff is observational in this version and does not update weights, biases, or future choices. Rock-Paper-Scissors labels are assigned to output assemblies by the model designer; the network does not understand the labels. Training and adaptation are deferred to Neural Strategy Adaptation V1. Prompt NUX1B fixes a Next production-build failure caused by build-time remote font fetches; it does not change engine semantics.
