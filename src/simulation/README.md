...
`src/simulation/runtime` provides one bounded port over authoritative engine ownership. `LocalRuntimeDriver` and `WorkerRuntimeDriver` share `RuntimeSession`, validated RunConfig/template construction, scheduler semantics, and projectors; they are not separate simulation implementations. The Worker imports trusted static ORTUS code only and exchanges strict Zod-validated protocol messages. The public runtime barrel exposes drivers, data contracts, and decoding only; host, session, scheduler, gates, and protocol internals remain private authorities.

`RenderFramePacket` is an ephemeral typed-array renderer projection with generation/run/template/projection/tick identity. It is not a snapshot, persistence format, evidence record, or `CanonicalObservation`. `UIProjection` is lower-frequency React/accessibility state and does not carry per-entity motion arrays. Same-generation frame/UI revisions must be positive and strictly increasing. The explicit `flocking-v1` projector supports only `flocking-boids`; both drivers reject unsupported templates, which require explicit audited projectors and rendering primitives.

Frame/UI publication each permits one in-flight value and one newest pending value. Coalescing can discard obsolete visual publications but never simulation steps, accepted commands, interventions, comparison captures, or future evidence records. All requests and fire-and-forget controls share a maximum of 128 unconsumed Worker messages, released only by host consumption acknowledgements; overflow rejects before acceptance. Worker buffers transfer ownership and are not recycled after detachment; shared memory and `OffscreenCanvas` are not used. Lifecycle is explicit as idle, initializing, ready, failed, or disposed. Commands before first readiness reject, and failure is terminal for that driver: it never falls back, restarts, or changes seed on its own. An explicit rebuild (Reset or a Setup change) retires a failed production driver and starts the requested run on a fresh driver and Worker. `importArtifact` first prepares the imported run on the calling thread with `prepareRuntimeArtifactImport` (parse, template check, engine construction, snapshot restore, invariant and template validation) and discards it; an artifact that preparation refuses is rejected before the generation changes, so the active run stays in place. The session repeats the same preparation when it commits the import.
...
Production Flocking uses `SimulationRuntimePort` and a dedicated `WorkerRuntimeDriver`. The Worker-owned `RuntimeSession` owns the engine, seeded RNG, scheduler, modeled state, and cadence; it transfers compact `RenderFramePacket` values to the batched Canvas and publishes bounded `UIProjection` plus selected-detail values to React. The continuous Flocking visual path does not construct or publish full snapshots. Explicit validated scenario and snapshot artifacts remain available for import, export, restore, and comparison operations. The other six production templates retain the mounted React animation-frame accumulator, Zustand controls, detached `SimulationSnapshotView`, and main-thread engine path. Neither path produces scientific observation, and only validated `SnapshotExport` data is exact continuation state.
...
`src/simulation/atlasPreview` is a headless bounded executor for GW9, audited and hardened by GW9B. Its explicit capability descriptor currently allows only `flocking-boids`, one bundled validated random-headings scenario, five numeric parameters, and three implemented numeric metrics. Requests use one or two deterministic inclusive axes, one to three unique signed integer seeds in canonical ascending order, a 1-250 tick horizon, at most 25 points, and at most 5,000 work units.
