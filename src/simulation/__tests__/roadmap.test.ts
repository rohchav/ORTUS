...
const requiredDocPhrases = [
  ...
  "A behavioral landscape describes how model behavior may vary across model conditions.",
  "It is not a real-world map, empirical proof, or Discovery Atlas record.",
  "Conceptual landscape anatomy is not sampled run data, and a future sampled landscape would still describe model behavior rather than empirical truth.",
  "A landscape probe plan describes how a future model-space investigation could be framed.",
  "It is not a sampled landscape, run queue, saved experiment, evidence record, or discovery.",
  ...
  "Existing local World comparison storage may preserve bounded run summaries for comparison.",
  "This is not persistent Lab evidence, not Atlas discovery storage, not saved behavioral landscapes, not saved probe plans, and not real-world validation.",
  ...
  "Lint is intentionally unavailable until a dedicated lint/tooling prompt adds it.",
...
describe("roadmap alignment and missing pillar reservations", () => {
  it("documents the revised roadmap and reserved missing pillars", () => {
    ...
    const behavioralLandscapeFoundationPath = join(repoRoot, "docs", "ui", "BEHAVIORAL_LANDSCAPE_EXPLORATION_FOUNDATION.md");
    ...
    const progressiveDisclosureArchitecturePath = join(
      repoRoot,
      "docs",
      "ui",
      "PROGRESSIVE_DISCLOSURE_AND_BEGINNER_ADVANCED_INFORMATION_ARCHITECTURE.md"
    );
    const progressiveDisclosureArchitectureAuditPath = join(
      repoRoot,
      "docs",
      "ui",
    ...
    const guidedBuilderFoundationAuditPath = join(
      repoRoot,
      "docs",
      "ui",
    ...
    const starterWorldFrameworkPath = join(
      repoRoot,
      "docs",
      "product",
    ...
    const starterWorldPortfolioPath = join(
      ...
      "docs",
      "product",
      "STARTER_WORLD_PORTFOLIO.md"
    );
    const exploreWorldsCatalogPath = join(
      repoRoot,
      "docs",
      "ui",
    ...
    const guidedInvestigationProductPath = join(
      repoRoot,
      "docs",
      "product",
    ...
    const runtimePerformanceArchitectureAuditPath = join(
      repoRoot,
      "docs",
      "performance",
      "RUNTIME_PERFORMANCE_ARCHITECTURE_AUDIT.md"
    );
    ...
    expect(existsSync(behavioralLandscapeFoundationPath)).toBe(true);
    ...
    expect(existsSync(runtimePerformanceArchitectureAuditPath)).toBe(true);
    ...
    const behavioralLandscapeFoundation = readFileSync(behavioralLandscapeFoundationPath, "utf8");
    ...
    const runtimePerformanceArchitectureAudit = readFileSync(runtimePerformanceArchitectureAuditPath, "utf8");
    ...
    expect(runtimePerformanceArchitecture).toContain("PERF1B, A0, A0B, I1, and I1B are complete.");
    expect(runtimePerformanceArchitecture).toContain("UR0 - Product Leverage + Comprehension Gate is next and unstarted.");
    ...
    expect(runtimePerformanceArchitectureAudit).toContain(
      "Verdict: `CONDITIONALLY READY FOR FUTURE I1 CONSUMPTION`"
    );
    expect(runtimePerformanceArchitectureAudit).toContain("No known P0 or P1 remains.");
    expect(runtimePerformanceArchitectureAudit).toContain("128 unconsumed transport messages");
    ...
    expect(localRulesCollection.match(/^## \d+\. /gm)).toHaveLength(15);
    ...
    expect(starterWorldPortfolio.match(/^### C\d+\. /gm)).toHaveLength(8);
    for (const field of [
      "workingTitle",
      "hookQuestion",
      "domain",
      "fictionalOrResearchContext",
      "entities",
      "environment",
      "resources",
      "networks",
    ...
    expect(roadmap).toContain("Prompt GW8B audits and hardens the Landscape Probe Planning Foundation");
    expect(behavioralLandscapeFoundation).toContain(
    ...
    expect(behavioralLandscapeFoundation).toContain("Conceptual scaffold - not sampled run data.");
    expect(behavioralLandscapeFoundation).toContain("GW7B is complete");
    expect(behavioralLandscapeFoundationAudit).toContain("Decision: ready for GW8");
    expect(behavioralLandscapeFoundationAudit).toContain("Actual browser zoom at 125%, 150%, and 200% was not verified.");
    expect(behavioralLandscapeFoundationAudit).toContain("The rendered landscape scaffold adds no fake interactive controls, route-local buttons, inputs, links, or static Tab stops.");
    expect(landscapeProbePlanningFoundation).toContain(
    ...
    expect(landscapeProbePlanningFoundation).toContain("Conceptual probe plan - not executable and not saved.");
    expect(landscapeProbePlanningFoundation).toContain("GW8B is complete");
    expect(landscapeProbePlanningFoundation).toContain("Planned comparison is not a comparison result.");
    expect(landscapeProbePlanningFoundationAudit).toContain("Decision: ready for GW9");
    ...
    expect(ephemeralLandscapePreview).toContain("GW9 adds the first real Atlas sampling capability");
    ...
    expect(sandboxVisualLanguageFoundation).toContain("UX4 implements the first bounded sandbox visual-language foundation");
    expect(sandboxVisualLanguageFoundation).toContain("Change visual language, not product capability.");
    expect(sandboxVisualLanguageFoundation).toContain("Actual browser zoom at 125%, 150%, and 200% was not verified.");
    expect(sandboxVisualLanguageFoundation).toContain("No runtime/template/Builder execution was added.");
    ...
    expect(sandboxVisualLanguageFoundation).toContain("UX6 and its UX6B audit followed and are now complete.");
    ...
    expect(progressiveDisclosureArchitecture).toContain("UX5 changes information architecture, copy layering, disclosure, and component-local view state");
    expect(progressiveDisclosureArchitecture).toContain("UX5 adds no new persistence.");
    expect(progressiveDisclosureArchitecture).toContain("Existing bounded World comparison and UI storage remain unchanged.");
    expect(progressiveDisclosureArchitecture).toContain("Existing local experiment sweeps remain implemented.");
    expect(progressiveDisclosureArchitecture).toContain("Atlas landscape sampling and landscape probe execution remain unimplemented.");
    expect(progressiveDisclosureArchitecture).toContain("Nothing on this route is a saved experiment, evidence record, notebook, or run history.");
    expect(progressiveDisclosureArchitecture).toContain(
      "No sampled landscape, saved map, probe execution, regime detection, or discovery record exists here yet."
    );
    expect(progressiveDisclosureArchitecture).toContain("Actual browser zoom at 125%, 150%, and 200% was not verified.");
    expect(progressiveDisclosureArchitecture).toContain("UX5B followed and is now complete");
    expect(progressiveDisclosureArchitecture).toContain("GW9 remains paused.");
    expect(progressiveDisclosureArchitectureAudit).toContain("UX5B complete.");
    expect(progressiveDisclosureArchitectureAudit).toContain("UX6 is next.");
    expect(progressiveDisclosureArchitectureAudit).toContain("GW9 remains paused.");
    expect(progressiveDisclosureArchitectureAudit).toContain("Ready for UX6.");
    ...
    expect(guidedBuilderFoundation).toContain("UX6B complete.");
    expect(guidedBuilderFoundation).toContain("GW9 is next.");
    expect(guidedBuilderFoundation).toContain("GW9 remains paused until UX6B is committed and remotely aligned.");
    expect(guidedBuilderFoundation.match(/^## \d+\. /gm)).toHaveLength(50);
    expect(guidedBuilderFoundationAudit).toContain("Guided Builder supports a bounded structural subset.");
    expect(guidedBuilderFoundationAudit).toContain("Advanced Builder remains the exact structural editing surface.");
    expect(guidedBuilderFoundationAudit).toContain("UX6 adds no Guided Builder persistence.");
    expect(guidedBuilderFoundationAudit).toContain("Guided Builder and Advanced handoff do not mutate World.");
    expect(guidedBuilderFoundationAudit).toContain("Actual browser zoom at 125%, 150%, and 200% was not verified.");
    expect(guidedBuilderFoundationAudit).toContain("npm run lint: unavailable, package.json has no lint script.");
    expect(guidedBuilderFoundationAudit).toContain("UX6B complete.");
    expect(guidedBuilderFoundationAudit).toContain("GW9 is next.");
    expect(guidedBuilderFoundationAudit).toContain("GW9 remains paused until UX6B is committed and remotely aligned.");
    expect(guidedBuilderFoundationAudit.match(/^## \d+\. /gm)).toHaveLength(51);
    ...
    expect(remoteStateMissionHygiene).toContain("Lint is intentionally unavailable until a dedicated lint/tooling prompt adds it.");
    expect(remoteStateMissionHygiene).toContain("Post-change rendered verification passed through the local-server permission path");
    ...
    expect(roadmap).toContain(
      "P0, UX0, UX1, and GW0 are complete as documentation-only prompts, UX2/UX2B are complete as bounded shared visual-foundation and rendered-audit prompts, GW1/GW1B are complete as the first bounded destination-shell implementation and audit pair, GW2/GW2B are complete as live World active-run context and audit, GW3/GW3B are complete as live World intervention readiness and audit, GW4/GW4B are complete as non-persistent Atlas foundation and audit, and GW5/GW5B are complete as non-persistent Lab evidence-record foundation and audit."
    );
    ...
    for (const futureName of ["GW0", "GW1", "GW2", "GW3", "GW4", "GW5", "GW6", "GW7", "GW8"]) {
  ...
  it("keeps README, concepts, simulation docs, product philosophy, and AGENTS aligned on future-pillar boundaries", () => {
    const docs = [
      ...
      readFileSync(join(repoRoot, "docs", "ui", "BEHAVIORAL_LANDSCAPE_EXPLORATION_FOUNDATION.md"), "utf8"),
    ...
    expect(docs).toContain("UX6B complete.");
    expect(docs).toContain("GW9 is next.");
    expect(docs).toContain("GW9 remains paused until UX6B is committed and remotely aligned.");
    ...
    expect(docs).toContain("No runtime/template/Builder execution was added.");
    ...
    expect(docs).toContain("GW8 landscape probe planning is non-executable and non-persistent.");
    expect(docs).toContain("Planned comparison is not a comparison result.");
    expect(docs).toContain("Do not add run-probe, run-sweep, save-probe, save-landscape, send-to-Lab, or publish-to-Atlas actions from GW8.");
    expect(docs).not.toContain("Prompt GW5 Behavioral Landscape Exploration");
